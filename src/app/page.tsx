...
export default function Home() {
  ...
  const handleDownloadCoupleCard = () => {
    ...
    const drawBox = (x: number, y: number, w: number, h: number, r: number = 24) => {
    ...
    drawBox(80, 685, 640, 195, 24);
    ctx.fill();

    ctx.fillStyle = '#4F46E5';
    ctx.font = 'bold 19px sans-serif';
    ctx.fillText(`오늘의 생활 주도권 (케미스트리 ${coupleResult.chemistryScore || 85}점)`, 400, 730);
  ...
  const [potionData, setPotionData] = useState<PotionDetail & color: link?: string string; { }>({
        ...
        {isEntered && (
          <div className="flex justify-between items-center bg-slate-50 px-4 py-2.5 rounded-2xl border border-slate-200 shadow-2xs mb-3">
            ...
            <span className="text-[11px] font-black text-indigo-600">
              {activeTabMode === 'parenting' ? '육아 모드 진행 중' : '커플 모드 진행 중'}
            </span>
          </div>
        )}
            ...
            {step === 'form' && (
              <form onSubmit={handleSubmit} noValidate className="space-y-3.5 flex-1 flex flex-col justify-between pt-1">
                <div className="space-y-3">
                  ...
                  <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 shadow-2xs space-y-1.5">
                    ...
                    <p className="text-[11px] text-slate-500 leading-relaxed pt-0.5">
                      ※ 원더윅스는 출생일 대신 <b>출산 예정일</b>을 기준으로 계산합니다.
                    </p>
                    {errors.dueDate && (
                ...
                <div className="pt-2 pb-1 space-y-2.5">
                  ...
                  >
                    <div className="break-keep space-y-0.5">
                      <span className="text-[10px] font-extrabold text-amber-200 uppercase tracking-widest block">
            ...
            {step === 'result' && (
              <div className="space-y-5 pb-4 pt-1 animate-fadeIn">
                ...
                >
                  ...
                  <BabyAnimalHybridMascot animalIndex="{characterAnimalIndex}" animalModifier="{animalModifier}" characterTitle="{characterTitle}" cheonganIndex="{characterCheonganIndex}" colorLabel="{colorLabel}"/>
                  ...
                  <div className="w-full space-y-4 bg-slate-50/80 rounded-2xl p-4 text-left border border-slate-200/80 shadow-2xs">
                    ...
                    <TemperamentStatBar '주변 '촉감 `${scores.curiosity}점은 activeTooltip="{activeTooltip}" careTip: categoryTag="탐색" description="{{" fillColor="#38BDF8" id="curiosity" levelInterpretation: meaning: name="우리 아이 호기심 레벨" onToggleTooltip="{handleToggleTooltip}" score="{scores.curiosity}" }} 가벼운 감각을 기분이 놀이책이나 눈을 다양한 대장 바깥 반응하고 반짝이며 변화에 뻗는 사물과 사운드북, 산책으로 새로운 성향이에요.', 소리나 손을 아주 유형이에요!`, 자극을 장난감에 적극적으로 좋아져요.', 주면 지적 탐험하려는 호기심 환경/>

                    <TemperamentStatBar '낮 '신체를 `${scores.energy}점은 activeTooltip="{activeTooltip}" careTip: categoryTag="체력" description="{{" fillColor="#4ADE80" id="energy" levelInterpretation: meaning: name="일일 활동 에너자이저" onToggleTooltip="{handleToggleTooltip}" score="{scores.energy}" }} 기초 놀아야 놀이나 동안 만족하는 발산하려는 밤잠을 성향이에요!`, 신나게 신체 쏟게 않고 에너자이저 에너지를 온몸으로 움직이고 움직이며 자요.', 즐겁게 지치지 충분히 터미타임으로 푹 해주면 활력이에요.',/>

                    <TemperamentStatBar 'infant' '갑작스러운 '새로운 '원더윅스 : ? `${scores.fussy}점은 activeTooltip="{activeTooltip}" ageMode="==" careTip: categoryTag="감각" description="{{" fillColor="#F43F5E" id="fussy" levelInterpretation: meaning: name="감정 및 수면 예민도" onToggleTooltip="{handleToggleTooltip}" score="{scores.fussy}" }} 감각 감정 고집은 공감해 과부하와 구간이에요.`, 급성장기 뇌 눈빛으로 느끼는 대해 도약기에 따뜻한 반응 보챔이나 비교적 상태를 섬세한 성장의 수준이에요.' 수준이에요.', 신호이니 아기가 아이가 안정적인 유지하는 자극에 정상 정서 주세요.', 차분하고 칭얼거림 편안한 현재 환경이나/>
                  </div>
                ...
                <div className="bg-white rounded-3xl p-5 sm:p-6 shadow-sm text-slate-900 space-y-4 border border-slate-200">
                  ...
                  <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100 space-y-2 text-left">
                    ...
                    <div className="p-3 bg-white rounded-xl border border-slate-200 space-y-1 shadow-2xs">
                      <div className="text-xs font-bold text-slate-900">육아 팁</div>
                  ...
                  <div className="space-y-4 animate-fadeIn">
                    ...
                    <div className="bg-white rounded-3xl p-5 sm:p-6 border border-slate-200 shadow-md space-y-4">
                      ...
                      <div className="p-4 bg-slate-50 rounded-2xl border border-slate-200/80 space-y-2 text-left">
                        <span className="text-xs font-black text-indigo-700 block">사주 오행 법정 최종 선요청하신 전체 코드를 기준으로, 기존 코드의 누락이나 겹치는 부분(중복 선언, 누락된 변수나 함수 등)을 모두 정리하고 완벽하게 작동하도록 통합 및 수정했습니다. 

복사해서 바로 사용하실 수 있도록 아래에 전체 코드를 제공해 드립니다.

---

### 수정 및 반영된 전체 코드 (`page.tsx`)
...
const SERVICE_LINKS = {
  youtube: '[https://www.youtube.com/@Hamin_Hayoon_day?utm_source=baby_sokpuli&utm_medium=lounge&utm_campaign=youtube](https://www.youtube.com/@Hamin_Hayoon_day?utm_source=baby_sokpuli&utm_medium=lounge&utm_campaign=youtube)',
  instagramProfile: '[https://www.instagram.com/hamin_hayoon_day/?utm_source=baby_sokpuli&utm_medium=lounge&utm_campaign=instagram](https://www.instagram.com/hamin_hayoon_day/?utm_source=baby_sokpuli&utm_medium=lounge&utm_campaign=instagram)',
  instagramDM: '[https://ig.me/m/hamin_hayoon_day?utm_source=baby_sokpuli&utm_medium=ai_teaser&utm_campaign=dm_inquiry](https://ig.me/m/hamin_hayoon_day?utm_source=baby_sokpuli&utm_medium=ai_teaser&utm_campaign=dm_inquiry)',
  coupangDefault: '[https://link.coupang.com/a/gZzbA8ezNk](https://link.coupang.com/a/gZzbA8ezNk)',
  parentHealing: '[https://link.coupang.com/a/gZAUwv5ctw](https://link.coupang.com/a/gZAUwv5ctw)',
};
...
const PARENT_HEALING_PRODUCTS = {
  level5: {
    ...
    subTitle: '지친 하루, 뻐근해진 승모근과 목을 시원하게',
    icon: '💆',
    link: '[https://link.coupang.com/a/gZzATurY6u](https://link.coupang.com/a/gZzATurY6u)',
  },
  level4: {
    title: '콜드브루 디카페인 파우치 세트',
    subTitle: '지친 오후 부모 멘탈을 채워줄 든든한 카페인 수혈',
    icon: '☕',
    link: '[https://link.coupang.com/a/gZzDh8XAE8](https://link.coupang.com/a/gZzDh8XAE8)',
  },
  level3: {
    title: '유기농 카모마일 릴랙스 티 세트',
    subTitle: '육퇴 후 복잡한 생각을 비우고 편안한 숙면을',
    icon: '🫖',
    link: '[https://link.coupang.com/a/gZzHmhAhae](https://link.coupang.com/a/gZzHmhAhae)',
  },
  level2: {
    title: '천연 아로마 롤온 힐링 테라피',
    subTitle: '하루 1분, 손목에 가볍게 바르는 스트레스 완화',
    icon: '🌿',
    link: '[https://link.coupang.com/a/gZzJINaiAK](https://link.coupang.com/a/gZzJINaiAK)',
  },
  level1: {
    title: '프리미엄 페어링 디저트 컬렉션',
    subTitle: '평화로운 육퇴 후 부부가 함께 나누는 달콤한 보상',
    icon: '🍪',
    link: '[https://link.coupang.com/a/gZzLYIshOK](https://link.coupang.com/a/gZzLYIshOK)',
  },
};

const BABY_POTION_LINKS: Record<'wood' | 'fire' | 'earth' | 'metal' | 'water', Record<'infant' | 'toddler' | 'child', string>> = {
  wood: {
    infant: '[https://link.coupang.com/a/gZxQNRGK0i](https://link.coupang.com/a/gZxQNRGK0i)',
    toddler: '[https://link.coupang.com/a/gZzTAoFGHA](https://link.coupang.com/a/gZzTAoFGHA)',
    child: '[https://link.coupang.com/a/gZzWiJKXxA](https://link.coupang.com/a/gZzWiJKXxA)',
  },
  fire: {
    infant: '[https://link.coupang.com/a/gZz0FSD20y](https://link.coupang.com/a/gZz0FSD20y)',
    toddler: '[https://link.coupang.com/a/gZz2WPe6cS](https://link.coupang.com/a/gZz2WPe6cS)',
    child: '[https://link.coupang.com/a/gZz5q33Qqa](https://link.coupang.com/a/gZz5q33Qqa)',
  },
  earth: {
    infant: '[https://link.coupang.com/a/gZz7vAV37s](https://link.coupang.com/a/gZz7vAV37s)',
    toddler: '[https://link.coupang.com/a/gZAB1mecDY](https://link.coupang.com/a/gZAB1mecDY)',
    child: '[https://link.coupang.com/a/gZADU7LZvw](https://link.coupang.com/a/gZADU7LZvw)',
  },
  metal: {
    infant: '[https://link.coupang.com/a/gZAF6nJOrk](https://link.coupang.com/a/gZAF6nJOrk)',
    toddler: '[https://link.coupang.com/a/gZAH1uI9zU](https://link.coupang.com/a/gZAH1uI9zU)',
    child: '[https://link.coupang.com/a/gZAJNeiG4q](https://link.coupang.com/a/gZAJNeiG4q)',
  },
  water: {
    infant: '[https://link.coupang.com/a/gZANoTDZEi](https://link.coupang.com/a/gZANoTDZEi)',
    toddler: '[https://link.coupang.com/a/gZAPp2bXVI](https://link.coupang.com/a/gZAPp2bXVI)',
    child: '[https://link.coupang.com/a/gZARxR6UJo](https://link.coupang.com/a/gZARxR6UJo)',
  },
};
...
export default function Home() {
  ...
  const handleShareResult = async () => {
    ...
    const shareTitle = `우리 집 육아 난이도: ${parentingDifficulty?.score || 80}점! [${parentingDifficulty?.level || '기질 분석'}]`;
    const shareDesc = `${name || '아이'}와 부모의 사주 오행 케미 및 실전 훈육 대화법을 확인해보세요 🍼`;
    const currentUrl = typeof window !== 'undefined' ? window.location.href : '[https://baby-sokpuli.vercel.app](https://baby-sokpuli.vercel.app)';
      ...
      try {
        (window as any).Kakao.Share.sendDefault({
          ...
          content: {
            title: shareTitle,
            description: shareDesc,
            imageUrl: '[https://baby-sokpuli.vercel.app/og-image.png](https://baby-sokpuli.vercel.app/og-image.png)',
            link: {
              mobileWebUrl: currentUrl,
              webUrl: currentUrl,
            },
              ...
              {isAnalyzingChemi ? (
                <div className="py-8 text-center space-y-5 animate-fadeIn">
                  ...
                  <div className="p-4.5 bg-black/40 border border-white/10 rounded-2xl space-y-3.5 shadow-inner">
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between items-center text-xs sm:text-sm font-bold">
                        <span className="text-blue-400">👨 아빠의 실시간 오행 기운</span>
                        <span className="font-mono text-blue-400">{dynamicDadBar}%</span>
                      </div>
                      <div className="w-full bg-white/10 h-4 rounded-full overflow-hidden p-0.5">
                        <div
                ...
                <form onSubmit={handleCalculateChemi} className="space-y-4 text-left">
                  <div className="space-y-1.5">
                    <label className="block text-xs font-bold text-amber-200">엄마 생년월일 (8자리)</label>
                    <input
                      type="text"
                      inputMode="numeric"
                      placeholder="예: 1993.05.10"
                      value={momBirth}
                      onChange={(e) => handleDateChange(e.target.value, setMomBirth, 'momBirth')}
                      className="w-full px-3.5 py-2.5 bg-white/10 rounded-xl border border-white/20 text-xs sm:text-sm text-white focus:outline-none"
                    />
                    {chemiErrors.mom && <p className="text-xs text-rose-400">{chemiErrors.mom}</p>}
                  </div>

                  <div className="space-y-1.5">
                    <label className="block text-xs font-bold text-amber-200">아빠 생년월일 (8자리)</label>
                    <input
                      type="text"
                      inputMode="numeric"
                      placeholder="예: 1991.12.25"
                      value={dadBirth}
                      onChange={(e) => handleDateChange(e.target.value, setDadBirth, 'dadBirth')}
                      className="w-full px-3.5 py-2.5 bg-white/10 rounded-xl border border-white/20 text-xs sm:text-sm text-white focus:outline-none"
                    />
                    {chemiErrors.dad && <p className="text-xs text-rose-400">{chemiErrors.dad}</p>}
                  </div>

                  <button
                    type="submit"
                    className="w-full py-3.5 bg-amber-400 hover:bg-amber-300 text-slate-950 font-black text-xs sm:text-sm rounded-xl transition-all shadow-md"
                  >
                    ⚖️ 오늘 육아 당번 판정하기
                  </button>
                </form>
                ...
                <div className="space-y-4 text-center animate-fadeIn">
                  <div className="p-4 bg-black/50 rounded-2xl border border-amber-500/30 space-y-2 text-left">
                    <span className="text-xs font-bold text-amber-400 block">✨ 법정 판결 결과</span>
                    <p className="text-base font-black text-white">
                      오늘의 육아 당번은 <span className="text-amber-300">{chemiResult.best}</span>입니다!
                    </p>
                    <p className="text-xs text-slate-300 leading-relaxed">
                      {chemiResult.summary}
                    </p>
                    <p className="text-[11px] text-purple-200 bg-white/5 p-2 rounded-lg">
                      {chemiResult.reason}
                    </p>
                  </div>
                  <button
                    onClick={handleCloseChemiModal}
                    className="w-full py-3.5 bg-amber-400 text-slate-950 font-black text-xs rounded-xl"
                  >
                    확인 완료
                  </button>
                </div>
